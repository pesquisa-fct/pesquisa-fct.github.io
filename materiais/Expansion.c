...
int main(){
	...
	double xm, ymDown, ymUp, ym, aInlet;
	int Nt, Nx, Ny;
	int Geometria, Malha;
						 ...
						 //					 /
						 //					 /
	//_______________________________________//	ymUp				 /
	//										 /
	//										 /
	//										 /
	//---------------------------------------//	ymDown				 /
	...
	for (j=0; j<=Ny; j++){
		...
		uOld[i][j] = uNew[i][j] = aInlet*(1.0/((ym - ymDown)*(ym - ymUp)))*( y[j] - ymDown )*( y[j] - ymUp );
		vOld[i][j] = vNew[i][j] = 0.0;
		psiOld[i][j] = psiNew[i][j] = aInlet*(1.0/((ym - ymDown)*(ym - ymUp)))*( ((y[j]*y[j]*y[j])/3.0) - (ymDown + ymUp)*((y[j]*y[j])/2.0) + ymDown*ymUp*y[j]);
		omegaOld[i][j] = omegaNew[i][j] = aInlet*( (2.0*y[j] - (ymDown + ymUp))/((ym - ymDown)*(ym - ymUp)) );
	}
	//PAREDES:
	...
	*/
	...
	while (n*dt < tf){
		...
		for (i=1; i<=im; i++){
			...
			psiOld[i][j] = psiNew[i][j] = aInlet*(1.0/((ym - ymDown)*(ym - ymUp)))*( ((ymUp*ymUp*ymUp)/3.0) - (ymDown + ymUp)*((ymUp*ymUp)/2.0) + ymDown*ymUp*ymUp);
		}
		j=je;
		for (i=im; i<=ie; i++){
			//psiOld[i][j] = psiNew[i][j] = -(3.0/(8.0*(ym*ym)))*( (yf/3.0) - (yf/2.0) )*yf*yf + c;
		...
		for (i=1; i<=im; i++){
			...
			//psiOld[i][j] = psiOld[i][j+1];
			//psiNew[i][j] = psiNew[i][j+1];
			psiOld[i][j] = psiNew[i][j] = aInlet*(1.0/((ym - ymDown)*(ym - ymUp)))*( ((ymDown*ymDown*ymDown)/3.0) - (ymDown + ymUp)*((ymDown*ymDown)/2.0) + ymDown*ymUp*ymDown);
		}
		j=j0;
		for (i=im; i<=ie; i++){
			//psiOld[i][j] = psiNew[i][j] = -(3.0/(8.0*(ym*ym)))*( (y0/3.0) - (y0/2.0) )*y0*y0 + c;
